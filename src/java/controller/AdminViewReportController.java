    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
         ...
         try{
             IReportDAO reportDAO = new ReportDAO();
              ...
              int count = reportDAO.getTotalReport();//7
            ...
            List<Reports> books = new ArrayList<>();
            books = reportDAO.getReportByName(name);
...
//            request.setAttribute("list", books);
     ...
     * The method is used to search all report by id and display paging
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
        ...
        try{
            ...
            if (request.getParameter("search") != null) {
             ...
             IReportDAO reportDAO = new ReportDAO();
              ...
              int index = Integer.parseInt(indexPage);
              int count = reportDAO.getTotalReportByTitle(name);//7
              int endPage = count / 3;//7/3=2
              if (count % 3 != 0) {
                  endPage++;
              }
            ...
            reports = reportDAO.getReportByName(name);
            request.setAttribute("searchname", name);
              List<Reports> listPage = reportDAO.pagingReportByTitle(name,index);
              request.setAttribute("listPage", listPage);
//            request.setAttribute("list", books);
              request.setAttribute("endPage", endPage);
